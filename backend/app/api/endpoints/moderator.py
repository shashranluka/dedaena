...
async def get_dedaena_data(
    ...
    """
    დედაენას ცხრილის სრული მონაცემების მიღება.
    აბრუნებს ყველა ტურს მათი სიტყვებით, წინადადებებით, ანდაზებითა და კითხვის ტექსტებით.
    """
    ...
    try:
        # ვიღებთ ყველა ტურს დალაგებულს position-ის მიხედვით
        result = db.execute(
            text(f"""
                SELECT 
                    id, 
        ...
        data = []
        for r in result:
            # JSONB ველების გარდაქმნა Python list-ებად
...
async def handle_dynamic_content_action(
    ...
    """
    ერთიანი endpoint-ი კონტენტის მართვისთვის (დამატება, განახლება, წაშლა).
    """
    ...
    # content_type-ს ვამზადებთ ბაზის სვეტის სახელისთვის (მაგ: 'word' -> 'words')
    db_column = f"{content_type}s"
    if db_column not in ["words", "sentences", "proverbs", "readings"]:
        raise HTTPException(status_code=400, detail="Invalid content type")
    
    # 'readings' სვეტი ბაზაში არის 'reading'
    if db_column == "readings":
        db_column = "reading"

    try:
        # 1. ვიღებთ მიმდინარე მონაცემებს ტურიდან
        result = db.execute(
            text(f"SELECT {db_column}, letter FROM {table_name} WHERE position = :pos"),
            {"pos": request.position}
        ).fetchone()

        if not result:
            raise HTTPException(status_code=404, detail=f"Tour with position {request.position} not found")

        current_items = result[0] or []
        tour_letter = result[1]

        # 2. ვასრულებთ მოქმედებას (action)
        if action == "add":
            if not request.content:
                raise HTTPException(status_code=400, detail="Content is required for adding.")
            updated_items = current_items + [request.content.strip()]
            message = f"'{request.content[:20]}...' წარმატებით დაემატა {content_type}s."
            ...
            if not (0 <= request.arrayIndex < len(current_items)):
                raise HTTPException(status_code=400, detail="Invalid arrayIndex.")
            
            current_items[request.arrayIndex] = request.content.strip()
            updated_items = current_items
            message = f"ელემენტი ინდექსზე {request.arrayIndex} წარმატებით განახლდა."
            ...
            current_items.pop(request.arrayIndex)
            updated_items = current_items
            message = f"ელემენტი ინდექსზე {request.arrayIndex} წარმატებით წაიშალა."
        
        else:
            raise HTTPException(status_code=400, detail=f"Invalid action: {action}")

        # 3. ვანახლებთ ბაზას
        items_json = json.dumps(updated_items, ensure_ascii=False)
        
        update_query = text(f"""
            UPDATE {table_name}
            SET {db_column} = CAST(:items_json AS jsonb)
            WHERE position = :position
        """)
